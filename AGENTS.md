...
- Production dependencies are committed in `vendor/`, which holds production packages only. Install the test and analysis tools with `composer tools:install` (into `tools/vendor/`, never committed) before running tests. Development packages go into `tools/composer.json`, never into the root `require-dev`.
...
- A page whose write keys off a posted field rather than `$op` puts `Forms::isUnverifiedRequest()` at the write and names the fields, for the same reason. Same for an operation that is also a *view* (`configuration.php?op=modulesettings`, `untranslated.php?op=list`): guard the write, not the op. **Test for the field first**: `if (Http::postIsset('x') && Forms::isUnverifiedRequest())`, never the other way round. The guard is not a pure predicate — a call that comes back true is filed as a security event — so a guard asked before anything is known to have been posted reports a refused state change on every ordinary view of the page.
- **The guard writes the log entry; the caller does not.** `Forms::isUnverifiedCoreOp()` and `Forms::isUnverifiedRequest()` record every refusal they decide, naming the page, the operation, the scope and the method. A caller that adds a line of its own puts the same event in the log twice. Pass `$context` when the caller knows something the guard cannot — *which* record was about to be deleted — and `$severity` when the refusal is not a warning (`rawsql.php`, `pages/user/user_del.php`).
