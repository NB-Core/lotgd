...
if ($op == "stats" || $op == "") {
    $sql = "SELECT sum(gentimecount) AS c, sum(gentime) AS t, sum(gensize) AS s, count(acctid) AS a FROM " . \Lotgd\MySQL\Database::prefix("accounts");
    ...
    $sql = "SELECT count(*) AS c, acct.acctid,acct.name AS referer FROM " . \Lotgd\MySQL\Database::prefix("accounts") . " INNER JOIN " . \Lotgd\MySQL\Database::prefix("accounts") . " AS acct ON acct.acctid = " . \Lotgd\MySQL\Database::prefix("accounts") . ".referer WHERE " . \Lotgd\MySQL\Database::prefix("accounts") . ".referer>0 GROUP BY " . \Lotgd\MySQL\Database::prefix("accounts") . ".referer DESC ORDER BY c DESC";
    $result = \Lotgd\MySQL\Database::query($sql);
    ...
    for ($i = 0; $i < $number; $i++) {
        ...
        $sql = "SELECT name,refererawarded FROM " . \Lotgd\MySQL\Database::prefix("accounts") . " WHERE referer = {$row['acctid']} ORDER BY acctid ASC";
        $res2 = \Lotgd\MySQL\Database::query($sql);
        $number2 = \Lotgd\MySQL\Database::numRows($res2);
        for ($j = 0; $j < $number2; $j++) {
            $r = \Lotgd\MySQL\Database::fetchAssoc($res2);
    ...
    $sql = "SELECT count(acctid) AS c, substring(laston,1,10) AS d FROM " . \Lotgd\MySQL\Database::prefix("accounts") . " GROUP BY d DESC ORDER BY d DESC";
    $result = \Lotgd\MySQL\Database::query($sql);
