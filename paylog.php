...
if ($op == "") {
    $sql = "SELECT info,txnid FROM " . \Lotgd\MySQL\Database::prefix("paylog") . " WHERE processdate='" . DATETIME_DATEMIN . "'";
    $result = \Lotgd\MySQL\Database::query($sql);
    while ($row = \Lotgd\MySQL\Database::fetchAssoc($result)) {
        $info = unserialize($row['info']);
        $sql = "UPDATE " . \Lotgd\MySQL\Database::prefix('paylog') . " SET processdate='" . date("Y-m-d H:i:s", strtotime($info['payment_date'])) . "' WHERE txnid='" . addslashes($row['txnid']) . "'";
        \Lotgd\MySQL\Database::query($sql);
    }
    $sql = "SELECT substring(processdate,1,7) AS month, sum(amount)-sum(txfee) AS profit FROM " . \Lotgd\MySQL\Database::prefix('paylog') . " GROUP BY month ORDER BY month DESC";
    $result = \Lotgd\MySQL\Database::query($sql);
    Nav::add('Months');
    while ($row = \Lotgd\MySQL\Database::fetchAssoc($result)) {
    ...
    $sql = "SELECT " . \Lotgd\MySQL\Database::prefix("paylog") . ".*," . \Lotgd\MySQL\Database::prefix("accounts") . ".name," . \Lotgd\MySQL\Database::prefix("accounts") . ".donation," . \Lotgd\MySQL\Database::prefix("accounts") . ".donationspent FROM " . \Lotgd\MySQL\Database::prefix("paylog") . " LEFT JOIN " . \Lotgd\MySQL\Database::prefix("accounts") . " ON " . \Lotgd\MySQL\Database::prefix("paylog") . ".acctid = " . \Lotgd\MySQL\Database::prefix("accounts") . ".acctid WHERE processdate>='$startdate' AND processdate < '$enddate' ORDER BY payid DESC";
    $result = \Lotgd\MySQL\Database::query($sql);
