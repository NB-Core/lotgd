...
$op = httpget('op');
if ($op == "keepalive") {
    $sql = "UPDATE " . \Lotgd\MySQL\Database::prefix("accounts") . " SET laston='" . date("Y-m-d H:i:s") . "' WHERE acctid='{$session['user']['acctid']}'";
    ...
    $sql = "DELETE FROM " . \Lotgd\MySQL\Database::prefix("news") . " WHERE newsid='" . httpget('newsid') . "'";
    \Lotgd\MySQL\Database::query($sql);
