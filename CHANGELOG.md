...
- Close the SQL `addslashes()` guard's blind spot for helpers that return a query. `SqlAddslashesUsageCheck` required a sink marker — `$sql`, `Database::query`, `executeQuery(` or `executeStatement(` — within six lines above and twelve below the `addslashes()` call, so a helper of the shape `return "SELECT * FROM t WHERE n = '" . addslashes($n) . "'";` went unreported: the execution happens at the caller, often in another file. Not a hypothetical shape — it is the one I wrote by accident while probing this checker during the test audit, and watching it pass is what turned the gap up. A line whose quote is followed straight away by SELECT, INSERT, UPDATE, DELETE or REPLACE is now a finding on its own. Deliberately narrower than "contains a SQL keyword", which was the first attempt and reported prose such as `"Please select '" . addslashes($name) . "' from the list"` — two false positives out of four probe cases. The tree stays at zero findings; five new cases pin both directions. One narrower gap remains and is named in the code: an escaped value assigned to a variable whose query is built more than twelve lines later.
