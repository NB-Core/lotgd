...
- Default `csrf_mode` to `enforce`. The reason it shipped as `log` — a transport that could not be verified — no longer holds now that the runtime is in the tree and the header was observed arriving end to end. A tab left open across the upgrade is refused once, then stops polling and reloads a single time to pick up a fresh token, instead of retrying every few seconds forever.
