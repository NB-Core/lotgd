...
- Two more button classes that no stylesheet defines, and a test so there is not a fourth. `motd-del` (the MoTD editor's Del) and `user-del` (the button that deletes a player's account) carried the same defect as `linkbutton` did, and the sweep that fixed `linkbutton`'s 17 sites missed them because it searched for that literal string rather than for the property. Asking the general question instead -- *which class handed to a button helper does no stylesheet define?* -- finds both. Both now carry `button`, so those two buttons render as the theme's rather than as browser chrome. `tests/Forms/ButtonClassesAreStyledTest` now reads every class passed to `Forms::postButton()` or `formActionButton()`, plus the defaults those helpers declare, and fails if none of the stylesheets defines a selector a `<button>` could match. That is deliberately stricter than "is the class defined anywhere": `motd` is defined in nine stylesheets and only ever as `a.motd`, which is exactly the defect the mail action bar was fixed for, and a laxer check would have called it healthy. The test asserts its own scanner found the call sites before concluding anything from their absence, and asserts that the check still rejects `motd` -- so loosening either one fails rather than going quiet.

- The bar under a mail message is a list of described controls rather than concatenated markup, and modules can contribute to it through a new `mail-read-actions` hook. Until now `pages/mail/case_read.php` had no hook of any kind and nothing could be added to that row. The list carries what a control *does* -- kind, url, label, optional confirmation and fields -- and the core decides how it looks, so a module's button inherits the row's class and its escaping without supplying either, and a posting one gets a CSRF field rendered into its form -- though that field is not a check, since `runmodule.php` validates nothing and the module's own write branch still has to ask. That direction is the point: `mailform`, the nearest existing hook, lets a module `rawoutput()` its own element and class, which is how the row came to hold three different elements wearing one anchor-only class in the first place. The rendering primitives are `Forms::linkButton()`, `Forms::disabledButton()` and `Forms::actionBar()`, beside the `postButton()` and `formActionButton()` that were already there; the entry's `kind` is stated and never inferred, because a URL does not say whether following it navigates or acts; `actionBar()` renders standalone forms and must not be called from inside another one, which its docblock now says outright. A malformed entry is skipped rather than rendered half-formed, so one bad module contribution costs only itself -- but a module that *returns* an empty array still empties the row, because `Modules::hook()` assigns each return value over the payload instead of merging it, and that is documented at the hook and pinned by a test rather than papered over. The rendered markup is unchanged: byte-identical to the previous output apart from the per-request CSRF token, which differs between any two runs anyway.

- `mail.php`'s Inbox/Write strip renders through `Forms::linkButton()`. Its label was interpolated into the markup unescaped, and it arrives from the `mailfunctions` hook, so it is module-supplied text.

- The `linkbutton` class is replaced by `button` at all 17 call sites (`taunt.php`, `titleedit.php`, `masters.php`, `deathmessages.php`, `creatures.php`, `donators.php`, `modules.php` ×6, `configuration.php` ×2, the two ban pages, clan membership). `linkbutton` was defined in **no** stylesheet in the repository — the same defect as `motd` on the mail page, but spread across the administration pages, where those buttons have always rendered as browser chrome instead of the theme's. No stylesheet is touched: `.button` is defined unqualified in every theme.

- One shape of control for the mail read view, instead of three. Reply, Forward, Previous, Next, Delete, Mark Unread and Report to Admin were an `<a>`, a `<button>` from `Forms::postButton()` and a hand-written `<input type='submit'>`, all wearing `class='motd'` -- which every theme in the repository, legacy and Twig alike, defines only as `a.motd`. The rule could not match the two that were not anchors, so three of the seven controls rendered as unstyled browser chrome next to styled links. They now carry `button mail-nav__link`, the class pair `mail.php` has always used for its own Inbox/Write strip: `button` is the one class every theme defines unqualified, so it lands on any element. **No stylesheet was touched** -- the CSS was never the defect. The two layout tables are gone with them: one wrote `cellspacing` into a `style` attribute, where it is not a property at all, the other used the presentational attributes, both used `nowrap="true"`, and the lower one was a three-row grid rather than the single row it appeared to be. In their place are two `div.mail-nav` flow containers, which wrap on a narrow screen because that class already says so. The upper bar now carries everything that navigates and the lower everything that acts on the message, so Reply and the adjacent-message links are no longer emitted twice. Deleting a message asks first, as every other delete in the tree does. Report to Admin is a `Forms::postButton()` carrying its payload in `$fields` rather than a hand-built form, which brings it a token and replaces its `htmlentities()` with `Escape::html()`; the labels of Reply, Forward and Report were interpolated unescaped, and translated strings come from a table `SU_IS_TRANSLATOR` writes. An adjacent-message label with nowhere to go is a disabled `<span>` holding its place rather than bare text with no element around it.

- Physical resistance no longer applies to a riposte. It was subtracted on the riposte branches, where the damage figure is negative, so a combatant's own resistance made their own counter-blow *harder* — symmetrically on both sides of the exchange. Over 50,000 measured rounds a creature with 10 physical resistance riposted for 10.93 where one with none riposted for 1.45: a resistant creature absorbed more **and** hit back seven times harder for it. Damping instead of amplifying was tried and rejected, because a riposte is already halved and therefore small, so any meaningful resistance floors it at zero and the mechanic disappears; dropping the term leaves the riposte at the halved margin times its damage modifier, independent of resistance. On a blow that lands, resistance is unchanged — the player's average landed hit moves 7.70 → 7.63. Players will feel this: high-resistance creatures become less dangerous, and high-resistance players lose a riposte bonus they were getting silently. `UPGRADING.md` carries the before/after table.

### Fixed

- Let a damaging aura actually kill a companion. The block that prints a companion's dying text and drops it from the party could not run, for two independent reasons: it tested `$companion['hitpoints']`, the copy the `foreach` made *before* the damage was written to `$companions[$name]`, and the entry condition above has already established that value is over zero -- and its one effect was an assignment to `$newcompanions`, which inside that method was an undeclared local nothing read (`battle.php` has a global of that name, which is what the author meant). Either alone was enough to make the feature inert, so a poison-shaped aura damaged companions, never announced a death, and left them in the party at negative hitpoints. Now the damaged value is read back, a companion driven to zero is removed, one flagged `cannotdie` is floored at zero and kept, and the dying text is read in both cases. Nothing shipped with this game sets both `regen` and `aura`, so no existing content changes behaviour; the repair is for third-party modules, which is why it was recorded rather than made silently when the branch was first covered. The farewell is rendered by the same code the ordinary death path uses, extracted as `Battle::announceCompanionDeath()` rather than left as a second copy: the aura's own version substituted no `{companion}`, forced the `battle` schema over whatever the companion declares, and said nothing at all for a companion without text of its own -- three ways a module's wording came out wrong on a path that had only just become reachable. `auramsg` is optional now for the same reason `cannotdie` is -- reading it unguarded put an "Undefined array key" on every companion an aura healed -- and an aura with no wording of its own adds nothing to the page, asserted by the healing run producing byte-identical output to a run with no companions at all.

- Show a regeneration buff's own message instead of a developer placeholder. The branch tested `effectgmsg` -- with a stray `g` -- and then printed `effectmsg`, so a buff carrying the correctly spelled key fell through to the fallback. Nothing in the tree has ever set `effectgmsg`, which is why the wrong branch was the only branch: every mystic-power player regenerating in combat read "Tons of damage, hosé" rather than the module's "You regenerate for N health." One character, and it had been shipping the placeholder to players for as long as the buff has existed.

- Stop an aura regeneration warning once per round for every downed companion. The condition that decides whether a companion at zero hitpoints can still be healed reads the optional `cannotdie` key, and read it unguarded -- but `battle.php` leaves a companion at zero in the party rather than removing it, so that is the ordinary state of a party mid-fight. Any companion built without the key produced an `Undefined array key` on every round for any party carrying such a buff. Guarded with `??`, which changes no outcome: an absent key already evaluated as false.
...
- Stop a test being able to destroy the checkout it runs in. `tests/cron_common_exception.php` renamed the repository's own `common.php` out of the way, dropped a throwing fixture in its place, and restored it from a shutdown handler. Two of those at once and the second rename moves the *fixture* onto `common.php.bak`, on top of the real file; the restore then puts the fixture back as `common.php` and the original is gone. Not hypothetical -- it happened here while measuring, and the interleaving reproduces it deterministically outside the repository. A kill between the rename and the handler leaves a checkout with no `common.php` at all. `cron.php` now runs from a directory of the test process's own: the fixture and a copy of `cron.php` (a copy, because PHP resolves symlinks for `__DIR__`, and it is `cron.php`'s own `__DIR__` that decides which `common.php` it loads), with everything else symlinked so the copy stays one file. Nothing in the repository is touched, so a crash costs a stale temp directory instead. Measured: eight concurrent runs, zero failures and the tree intact, against one failure in eight before. The same test also owned `logs/bootstrap.log` -- deleting it at both ends of a file *any* subprocess in the suite appends to -- so `BootstrapErrorHandler` gained a `LOTGD_BOOTSTRAP_LOG` override and the test hands its subprocess a log of its own. The default path is unchanged, which matters for the Docker image, where the directory is root-owned and entries fall back to PHP's error log; an operator can now point it somewhere writable instead.

- Answer the CSRF question by running the page. Sixteen root pages guard their state-changing operations with `Forms::isUnverifiedCoreOp($op, [...])`, and every test of that guard reads the page as text: that the call appears near the top of the file, that its op list has the right entries. That holds just as well when the guard is unreachable, when the page exits before it, or when the op it names is not the op the page dispatches on -- none of which a substring search can tell from success. Eleven pages now execute for real, in their own process, and the test looks at the SQL: with a valid token `masters.php` issues its `DELETE FROM masters`, without one it does not, and the response is a 400. Each of the eleven had its guard disabled by mutation and each failure landed on that page's own rows. Getting a root page to run at all is what the work was, and every piece of the harness is one failed attempt fixed rather than asserted around -- forced navigation redirects any request the previous page did not offer and replaces `$session['user']` from the database first, so the fixture has to supply the account row and the allowednavs key built the way `sanitizeUri()` builds it; the page runs from a directory that looks like an installed game, `installer.php` absent and `dbconnect.php` present, because `common.php` exits on either; and the account row is derived from `get_all_tables()` and cast by declared column type rather than written out, since a hand-written one fails on whichever column it forgot and the string defaults give `Mounts::loadPlayerMount(int $horse)` a TypeError. The pairing is the design: "no write without a token" is satisfied by a page that never ran, and four earlier versions of this harness were exactly that, so every row asserts the write happening before it asserts it not happening. Two limits are stated in the file rather than left as absences -- the guard's `$_POST = []` has no witness here (removing it from each of the eleven pages in turn leaves all 33 cases green, eleven times over, because blanking `$op` alone already stops these dispatches), and five of the sixteen pages are absent because they act through the session or the settings object and leave no statement that distinguishes their operation from a page view. `bank.php` is the one that matters most of those, so that is a gap and not a decision; the source-text tests stay in place for the pages this cannot reach. Separately, the `Database` test double had drifted from the class it aliases itself over -- five missing public methods, unnoticed because no test had ever run a path that reaches them, and the first one that did got "Call to undefined method" from inside `common.php`, which surfaces as a rendered error page rather than a test failure.

- Cover the regeneration branch of `Buffs::activateBuffs()`, the last uncovered branch on the audit's backlog, and find two bugs in it. It needed no seam, which is worth recording because the three items before it on this list each did: unlike the minion branch below it, the regen arithmetic draws no randomness and reads only globals the fixture already sets, so it could simply be run. 20 cases, each proved against a mutation that fails it and, for most, fails only it. What they pin is the clamp on both sides -- a heal stops at maximum or a regen buff is an unbounded hitpoint fountain, and the separate `$hpdiff < 0` guard is what stops a player who is *over* maximum from being pulled back down by a buff meant to help -- plus the two gates (`roundstart` only, and only against the enemy actually targeted), the aura's third-strength extension to companions with its per-companion clamp, and the rounding floor that keeps a regen of 1 from announcing a heal of 0 to the whole party. A guard is two claims, and the first version of this wrote down only one: the `cannotdie` clause had a case for the companion *without* the key and none for the companion the flag exists to serve, so the whole `|| (cannotdie && $auraeffect > 0)` alternative could be deleted with all twenty cases still green -- companions that cannot die simply never healed again. Found by Codex, reproduced before fixing, and the third time in this audit that I tested a guard by its default and not by its exception. Both halves have a case now. One finding is recorded rather than repaired, because repairing it would change what third-party modules do: the block that prints a companion's dying text and drops it from the party cannot run. It tests the by-value copy the `foreach` made *before* the damage was written, so it always sees the pre-damage value; and its one effect is an assignment to `$newcompanions`, which inside this method is an undeclared local that nothing reads -- `battle.php` has a global of that name, which is presumably what was meant. Both halves were measured: making the first live fails the test that documents today's behaviour, and making the second live alone changes nothing at all.

- Finish the grep-test audit, and find one of my own replacements was a tautology. `RequestNormalizationRegressionTest` was eleven substring searches over six pages. The negatives are enforced tree-wide by the interpolation guard; the `rawurlencode(...)` assertions guarded belt over braces, since `healer.php` has already matched the value against a two-entry allowlist by then; and the narrowings themselves are tested where they live. What was left is *wiring*, and #1533 is why that survives rather than going with the rest -- dropping a wiring witness without replacing it let the game's only automatic ban be turned off with the whole suite green. `tests/Support/SourceFlow.php` asks those questions structurally: the value handed to the link builder must be one the guard produced, and the variable tested with `ctype_digit()` must be the one read from the request. `AccountRestorepageTest` checked three files with `/restorepage.*village\.php/s`, whose `s` modifier spans the whole file -- any mention of each, hundreds of lines apart, satisfied it; the schema is data and is now read, and the default page is asserted to exist, which every earlier assertion would have survived a rename of. The `color_sanitize` delegation check moved out of `UtilityTest` entirely: `tests/Stubs/Functions.php` defines that name behind `function_exists()` and wins, so an in-process equivalence test compares the class with itself and passes however `lib/sanitize.php` is written. My first replacement did exactly that and a mutation exposed it. It runs in an isolated process now, like the `lib/http.php` wrappers, and asserts by reflection that the function under test really is the file on disk.
- Widen the SQL interpolation guard's reach and stop it vetoing a query for its own markup. The audit only ever read `src`, `lib`, `pages`, `async` and the root entry points, so `install/`, `modules/`, `migrations/` and `scripts/` -- 77 production files -- had never been scanned at all; one of them, `install/lib/Installer.php`, builds a lookup from a posted username, and the tree's count moves from 118 to 119 because of it. `--changed-since` now skips only `tests/`, whose fixtures hold the forbidden shapes deliberately. Separately, `looksLikeSql()` rejected any text containing an HTML tag, so an `INSERT` that stores formatted content hid an injected value behind its own `<p>`; a whole statement is now recognised regardless of markup, and the veto is kept only for a bare keyword, which is what `<select name=…>` needs it for. Both reported by Codex while reviewing the concatenation rule.
- Retire three more grep tests by building what each was gesturing at. `HighRiskSqlMigrationTest` and `UntranslatedParameterBindingRegressionTest` each had two halves and both were empty. The source half searched `superuser.php`, `translatortool.php` and `untranslated.php` for query spellings. The other half looked behavioural but was a tautology: the test built a SQL string itself, handed it to the `DoctrineConnection` stub, and asserted the stub had recorded what it had just been given -- no production code took part. Proved rather than argued: with all three pages deleted from the tree those methods still pass, 11 assertions green, while the source halves fail as they should. What the greps were reaching for now exists. The negative assertions -- no request value concatenated into a query -- are enforced tree-wide by the extended interpolation guard. `. LANGUAGE .` is replaced by a test of the sanitiser it depends on: `LANGUAGE` is built from a cookie or an account preference and is concatenated into queries rather than bound, so the defence is `Translator::normalizeLanguageCode()` stripping it to letters, which had no test at all while the spelling did. And the cache-key assertions froze a duplication instead of removing one: `untranslated.php` carried its own copy of Translator's key builder, with a comment asking the reader to keep them aligned, and the test compared the two spellings. `Translator::translationCacheKey()` is the only copy now, with the hashing boundary asserted from both sides and the collision that hashing prevents asserted at all -- a case two matching source strings could never make.
- Weigh the automatic login ban somewhere it can be tested, and then test it. Ten failed logins from one address inside a day ban it for fifteen minutes, and a failure against a superuser account counts twice -- so five of those are enough, which is what `login.php`'s comment has always claimed and what nothing ever checked. The rule sat inline in the middle of the failed-login branch, in a loop that also built the alert mail, and the test covering this page read the page as text: it asserted that the string `if ($c >= 10)` appeared in it. That holds just as well when the counter is wrong. `Lotgd\Security\LoginFailureTally` now weighs the run and answers `warrantsBan()`; 13 cases pin the threshold from both sides, the doubled weight, and the case that only bites on one of the two database paths -- the driver hands integer columns back as strings, so a `superuser` flag of `"1"` has to weigh the same as `1`. Five mutations, each failing its own case. The rest of `LoginQuerySecurityTest` went from seventeen substring searches to two: the ones asserting no value is interpolated into the login query are enforced tree-wide by `scripts/check-sql-value-interpolation.php` -- measured, by reintroducing `WHERE login = '$name'` into a copy of the real file and watching the scan report it -- and the password checks are already exercised by `tests/PasswordHelperTest.php`. What survives is the faillog column list, rewritten to check a relationship instead of a spelling: it runs `get_all_tables()` and asserts every column the page writes is one the schema defines, on both database paths, so a rename on either side is caught. That one has failed in production before -- the column is `id` and an older revision wrote `lgi`, and because the INSERT sits inside a try/catch that must not break login, a wrong name surfaces as failed logins silently going unrecorded, taking the ban that reads them back with them.
- Give the bank an arithmetic that can be tested, and then test it. `bank.php` moves every gold piece a player owns — deposit, withdrawal, loan and transfer between accounts — and had **no tests at all**, because all four sat inline between output lines, reading `$session` directly and writing straight back. `Lotgd\Bank` now holds the three operations that move money, with nothing read from global scope and nothing printed; the page keeps every message it had and picks it from a typed result. A refused operation hands back the balance it was given unchanged, so a caller cannot half-apply a rejection. Verified by replaying **10,125 combinations** of cash, balance or debt, level, daily total, amount and recipient shape against reference functions transcribed verbatim from the old page — zero divergences, refusals included. 39 new cases, proved against ten mutations. Two things surfaced: a debt blocks a transfer the cash in hand could pay for on its own (the cover check is cash *plus* a signed balance), and `bank.php:271` held an undefined-variable typo in a branch that is provably unreachable — its condition is the exact negation of the one guarding it, and the cross product never entered it once.
- Lift the level-up out of `train.php` and cover it. Beating your master is the only way a character's level ever goes up, every gain is permanent, and none of it was tested — it sat inline between output lines. `PlayerFunctions::levelUp()` and `levelUpCompanion()` take the character and return it, so a caller that forgets to assign gets no level-up rather than half of one. 17 new cases, six mutations. Verified over 148 combinations — every level-up shape and all 128 subsets of the companion keys — against functions transcribed verbatim from the old page: zero divergences. One slip reproduced rather than repaired, because fixing it would change behaviour: the companion healing step tests for `attack` and then assigns from `maxhitpoints`, so a non-fighting companion is never healed and a fighting one without a maximum gets a warning and a null. Two tests pin both halves and name themselves as the place it would be restated — the mutation that repairs it fails exactly those two.

- Make the suite prove something. It was green, but only in the order PHPUnit happens to walk the files in: reversed it produced 28 failures and 15 errors, and three random seeds produced 11, 26 and 27. A green run was evidence about the ordering, not about the code — and one file was named `AMotdTest.php` purely to win the alphabetical race, which is the dependency written down as a filename. Two causes. `tests/Stubs/Database` aliases itself over `Lotgd\MySQL\Database` for the whole run and holds 19 public statics, and `Translator`, `DataCache`, `Settings`, `Output`, `Modules` and `Mail` keep their own; nothing reset them between tests, so each test inherited whatever the last one left. A PHPUnit extension now resets all of it on `PreparationStarted`, before each test's own `setUp`, so fixtures still land the way they did — wired as an extension rather than a base class because no test can then opt out by forgetting `parent::setUp()`. The other cause is constants: fourteen classes `define()` process-global values such as `DB_CHOSEN`, `LANGUAGE` and `DB_NODB`, and a constant cannot be undefined, so whichever ran first decided it for every test after it. Those run in their own processes now. CI gained a second run with `--order-by=random`, because both causes come back silently otherwise.
- Stop tests reporting green when they had not run. `FreshInstallTest` called `markTestSkipped('Installer failed to run')` on a non-zero exit — the one outcome it exists to catch — and the two `ErrorHandler` tests that shell out asserted on stdout without checking the exit status, so a script that died before reaching its own output looked the same as one that passed. Isolating the shared state also exposed four tests that had been passing on state borrowed from their neighbours, each asserting something untrue: one whose ajax branch had never been entered because it set the `$settings` global while the code reads the singleton, one that claimed an exception before `Settings` exists sends a notification when `errorNotify()` deliberately bails out, one relying on a pre-warmed translation table, and one pinning a sentinel string the real `Database` can never return. Fixed at the source rather than by restoring the leak.
- Replace look-alike test classes with tables that say what they cover. Nine classes exercised 15 of the 45 one-line forwards in `lib/modules.php`, each in its own process with its own hand-written recording double; they are one provider-driven class covering all 45, verified against mutations that drop an argument and repoint a call. Nine `ErrorHandler` notification classes — several differing only in one errno or one absent setting, two of them asserting byte-identical strings — became two, and every row now also asserts that building the notification raises nothing of its own. Three genuine duplicates went (two `MassModulePrepareTest`s differing in their string literals, two `SprintfTranslateTest`s testing the same method with the same harness). Net: 230 test files to 213, with 5,153 assertions rising to 5,315.
- Make the combat damage roll testable, and then test it. `Battle::rollDamage()` is what every fight in the game runs through — forest, PvP, the dragon — and it had no tests, for a mechanical reason rather than a careless one: between it and its near-twin `rollCompanionDamage()` they read eleven globals and draw from `random_int()`, which is a CSPRNG and cannot be seeded. Any test of the old code was either flaky or asserted only that nothing threw. The arithmetic now lives in `computePlayerDamage()` and `computeCompanionDamage()`, which read nothing from global scope and take their randomness through a `RandomSource`; `rollDamage()` and `rollCompanionDamage()` keep their signatures, their return shapes and every global they read or write, because `lib/battle-skills.php` forwards the first as module API. The refactor was verified by replay rather than by reasoning — the same fixtures fought under a fixed `mt_srand()` before and after produced 30 identical rolls. 37 new cases follow, the load-bearing one being that the damage modifiers are **crossed**: a blow is scaled by the side that lands it, so a hit takes `dmgmod` and a riposte takes `badguydmgmod`, and swapping either pair would rebalance every fight in the game invisibly. Two findings recorded rather than fixed, since both would be behaviour changes: the player's re-roll loop has no escape hatch where the companion's gives up after 50 tries, and physical resistance makes a riposte *harder* rather than softer because the subtraction runs on a negative number.

### Security

- Close the SQL `addslashes()` guard's blind spot for helpers that return a query, by answering the SQL question from the token stream instead of the line text. `SqlAddslashesUsageCheck` required a sink marker — `$sql`, `Database::query`, `executeQuery(` or `executeStatement(` — within six lines above and twelve below the call, so a helper of the shape `return "SELECT * FROM t WHERE n = '" . addslashes($n) . "'";` went unreported: the execution happens at the caller, often in another file. The check now walks the `.`-concatenation around the call and asks whether the assembled literal text *opens* a statement. Reading tokens rather than lines is what makes that safe — a string literal is one token however many lines it spans, so multi-line SQL is seen; and quotes inside it are content rather than delimiters, so prose such as `"Choose 'delete' to remove "` is not mistaken for a query. The sink-based branch is untouched and still catches statements that do not open with a verb. The tree stays at zero findings.
- Close a blind spot in the SQL interpolation guard that let 35% of its own findings through CI. `SqlValueInterpolationCheck` attributed every finding to the line the string *opened* on, because a plain `"` token carries no line number and the reader walked backwards to the previous token. `--changed-since`, which is the mode CI runs, keeps only findings whose line the diff marked as added — so an interpolation reintroduced into the query argument of a multi-line `executeStatement(` call, whose opening line is untouched, was filtered out as "not added by this change". That is the shape most SQL in this codebase is written in. Demonstrated by reintroducing a real one: `masters.php` went back to `DELETE … WHERE creatureid = '$id'`, the full scan reported it, and the CI mode said **passed**. Findings now carry the line of the interpolated value itself. The tree's finding count is unchanged at 120 across the same files — but **42 of those 120 line numbers were wrong**, and each was a case CI would have waved through. The checker's own tests asserted the file and never the line, which is how it survived; three cases now pin the attribution for multi-line, curly-brace and single-line statements.
- Give `bank.php` the CSRF guard every other state-changing core page already had. Eighteen pages carry `Forms::isUnverifiedCoreOp()` at their entry; the page that moves a player's entire fortune was not among them, so a forged POST to `bank.php?op=transfer3` carrying `to` and `amount` moved the victim's gold to whoever sent it. `SameSite=Lax` keeps an ordinary cross-site POST from carrying the session cookie, which is why this was not trivially exploitable from the open web — but modules render into core pages through hooks and post forms of their own, and an operator running `SameSite=None` has no such protection. The guard sits once at the entry and names `transfer2`, `transfer3`, `depositfinish` and `withdrawfinish`; every form posting to one of them now emits `Forms::csrfField()`. Reported by Codex on #1528.
- Stop a posted bank amount of `-9223372036854775808` crashing the page. `abs(PHP_INT_MIN)` is a float, because the positive counterpart does not fit in an int, and a float against the extracted methods' `int` parameters is a `TypeError` under `strict_types` — a crash a player could ask for, introduced by typing those parameters. `Bank::postedAmount()` now normalises all four call sites and saturates at `PHP_INT_MAX`, which keeps the old outcome: a figure that large was refused for exceeding the balance before, and still is. Reported by Codex on #1528.
- Bind the bank transfer's `UPDATE` instead of interpolating it. `bank.php:175` built `SET goldinbank=goldinbank+$amt … WHERE acctid='{$row['acctid']}'` as a string. Neither value was reachable — `$amt` had been through `(int)` and `acctid` came from the page's own query — but it is exactly the shape `scripts/check-sql-value-interpolation.php` exists to stop, and a guard that has to make exceptions for the code it ships with is a guard nobody trusts. It is a bound Doctrine statement now, and the scanner no longer reports this file.
- Bind the referral bonus in `train.php` instead of interpolating it. `train.php:236` built `SET donation=donation+<award> WHERE acctid={$session['user']['referer']}` as a string. The award had been through `(int)` and the account id comes from the session rather than a request, so nothing was reachable — but it is the shape `scripts/check-sql-value-interpolation.php` exists to stop. The scanner reported two findings in this file and now reports one, the master-id `SELECT` that carries an in-code comment explaining its deliberate cast.

- Normalise request values through one guard instead of two that had drifted. `deathmessages.php` and `taunt.php` each carried their own id and text normalisers for values headed into bound SQL parameters, written to do the same job — but the deathmessages id guard cast to string before testing for digits, so a boolean `true` or a float `1.0` became the row id `1`, while the taunt one rejected both. `Http::get()` yields strings from a real request, so nothing was reachable through the web; two guards with one purpose and two behaviours are a bug waiting for a caller. `Lotgd\Security\RequestValue` now holds both, taking the stricter reading that both pages' own comments described. Writing it under test turned up a third case neither page handled: a digit string longer than `PHP_INT_MAX` saturates on cast rather than failing, so `"<PHP_INT_MAX>0"` became `PHP_INT_MAX` and addressed whatever row held it — an id must now survive the round trip. The two tests that had covered this read the pages as text and asserted on source substrings, which is how the drift survived: the spelling `ctype_digit(...)` matched in both places while the behaviour did not. The binding shape they also pinned is covered better by `scripts/check-sql-value-interpolation.php`, which tokenises rather than greps and runs in CI against changed lines.
- Give the core's state-changing operations the same CSRF guard, in one shape. `Forms::isUnverifiedCoreOp($op, [...])` sits at each page's entry, after it reads `$op`, and turns a POST without this page's form token into a plain page view by clearing `$op` and the body. At the entry rather than in each branch: a per-branch check is one somebody can forget — which is how a dozen editors came to write with no token at all — and a delete keys off `$op` with its id in the query string, so emptying the body alone would not stop it. The op list is a boundary rather than bookkeeping: modules render into these pages through hooks and `runmodule.php` carries no guard, so an `$op` the core does not implement passes through untouched and old modules keep working. Pages whose writes key off a posted field instead (`viewpetition.php`, the clan pages), and operations that are also views (`configuration.php?op=modulesettings`, `untranslated.php?op=list`), guard at the write. A GET is unverified like any other request without a token, so the remaining state-changing links — activating, deactivating, installing and reinstalling a module, marking mail unread, granting donation points — became buttons too. An editor whose token must not be interchangeable passes its own scope. Fifteen pages that had no check gain one.
...
- Companions now gain levels, which they never did. `train.php` computed each companion's new attack, defence and maximum hitpoints into `$newcompanions` and then never read that variable — not assigned back, not serialised into the session — so the `companionslevelup` setting, on by default, has been a no-op for every release it has shipped in. Reported by Copilot on #1529 and confirmed against `origin/master` before changing anything. Over a climb from level 3 to 15 a companion now gains roughly **240–260% attack** where before every figure stayed where it was hired; it is also healed to its new maximum on each master defeat. Set `companionslevelup` to `0` to keep the old behaviour — the switch now does what its name says in both positions. Two defects in the same block go with it, both invisible while the result was discarded: the healing step tested for an `attack` key and then read `maxhitpoints`, so a non-fighting companion was never healed and a fighting one without a maximum had its hitpoints set to `null` with an undefined-key warning. `UPGRADING.md` carries the table.
