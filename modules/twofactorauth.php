 ...
 *
 * Compatibility note: different Jaxon bootstrap versions emit handler objects under
 * getJaxonHandlers(), window.Lotgd.Async.Handler, or window.JaxonLotgd.Async.Handler.
 * We keep the lookup deterministic and only return a handler when the requested method exists.
 */window.twofactorauthResolvePasskeyHandler=window.twofactorauthResolvePasskeyHandler||function(method){const resolvedHandlers=typeof window.getJaxonHandlers==='function'?window.getJaxonHandlers():null;const requestedMethod=String(method||'');const candidates=[{name:'window.getJaxonHandlers()',handler:resolvedHandlers&&resolvedHandlers.TwoFactorAuthPasskey?resolvedHandlers.TwoFactorAuthPasskey:null,rootPresent:resolvedHandlers!==null&&resolvedHandlers!==undefined},{name:'window.Lotgd.Async.Handler',handler:window.Lotgd&&window.Lotgd.Async&&window.Lotgd.Async.Handler?window.Lotgd.Async.Handler.TwoFactorAuthPasskey:null,rootPresent:!!(window.Lotgd&&window.Lotgd.Async&&window.Lotgd.Async.Handler)},{name:'window.JaxonLotgd.Async.Handler',handler:window.JaxonLotgd&&window.JaxonLotgd.Async&&window.JaxonLotgd.Async.Handler?window.JaxonLotgd.Async.Handler.TwoFactorAuthPasskey:null,rootPresent:!!(window.JaxonLotgd&&window.JaxonLotgd.Async&&window.JaxonLotgd.Async.Handler)}];for(let i=0;i<candidates.length;i++){const candidate=candidates[i];if(candidate.handler&&typeof candidate.handler[requestedMethod]==='function'){return{handler:candidate.handler,source:candidate.name,requestedMethod:requestedMethod,sources:candidates};}}return{handler:null,source:'none',requestedMethod:requestedMethod,sources:candidates};};window.twofactorauthJaxonPasskeyCall=window.twofactorauthJaxonPasskeyCall||function(method,args){return new Promise(function(resolve,reject){const preflight=window.twofactorauthPreflightPasskeyRequestUri(method);if(!preflight.ok){const message='Passkey async endpoint mismatch.';const detail=preflight.detail||'';reject(new Error(showDebug&&detail!==''?message+' '+detail:message));return;}const resolution=window.twofactorauthResolvePasskeyHandler(method);const namespace=resolution&&resolution.handler?resolution.handler:null;if(!namespace){const sources=resolution&&Array.isArray(resolution.sources)?resolution.sources:[];const hasAnyRoot=sources.some(function(source){return!!(source&&source.rootPresent);});if(showDebug){const sourceSummary=sources.map(function(source){const hasHandler=!!(source&&source.handler);const hasMethod=!!(source&&source.handler&&typeof source.handler[String(method)]==='function');return String(source&&source.name?source.name:'unknown')+' rootPresent='+String(!!(source&&source.rootPresent))+' handlerPresent='+String(hasHandler)+' methodPresent='+String(hasMethod);}).join('; ');const diagnosticLabel=hasAnyRoot?'Passkey async export missing from generated Jaxon script':'Passkey async handler unavailable';emitDebug(diagnosticLabel,function(){return'method='+String(method)+' resolvedSource='+String(resolution&&resolution.source?resolution.source:'none')+' '+sourceSummary;});}const missingExportMessage=hasAnyRoot?'Passkey async export missing from generated Jaxon script.':'Passkey async handler unavailable.';reject(new Error(missingExportMessage));return;}const requestId='tfa_'+Date.now()+'_'+Math.random().toString(16).slice(2);const timeoutId=window.setTimeout(function(){rejectPending(requestId,'Passkey async request timed out.','method='+String(method));},20000);window.twofactorauthPendingRequests[requestId]={resolve:resolve,reject:reject,timeoutId:timeoutId,method:method};emitDebug('Request created',function(){return'id='+String(requestId)+' method='+String(method)+' source='+String(resolution&&resolution.source?resolution.source:'unknown');});const params=Array.isArray(args)?args.slice():[];params.unshift(requestId);try{namespace[method].apply(namespace,params);appendClientUriDiagnostic('Passkey async dispatched');}catch(error){rejectPending(requestId,'Passkey async request dispatch failed.',error&&error.message?String(error.message):'unknown');}});};if(!window.__lotgdTwofactorPasskeyBridgeRejectionHooks){window.__lotgdTwofactorPasskeyBridgeRejectionHooks=true;window.addEventListener('unhandledrejection',function(event){const reason=event&&event.reason&&event.reason.message?String(event.reason.message):'';if(reason.indexOf('Unexpected end of JSON input')!==-1||reason.indexOf('JSON')!==-1){window.twofactorauthRejectAllPending('Passkey async parser failure.',reason);}});window.addEventListener('error',function(event){const message=event&&event.message?String(event.message):'';if(message.indexOf('Unexpected end of JSON input')!==-1){window.twofactorauthRejectAllPending('Passkey async parser failure.',message);}});}})();</script>
JAVASCRIPT;
