...
function twofactorauth_render_challenge(Output $output): void
{
    ...
    if ($passkeys !== []) {
        ...
        $csrfEscaped = addslashes(twofactorauth_csrf_token());
        ...
        rawoutput("<script>(function(){const btn=document.getElementById('twofactorauth-use-passkey');if(!btn){return;}btn.addEventListener('click',async function(){try{const beginData=await window.twofactorauthJaxonPasskeyCall('beginAuthentication',['" . $csrfEscaped . "']);if(!beginData||!beginData.ok){alert('Passkey challenge failed.');return;}const publicKey=window.twofactorauthDecodeCredentialOptions(beginData.options.publicKey);const cred=await navigator.credentials.get({publicKey});if(!cred){alert('Passkey not available.');return;}const body={id:cred.id,type:cred.type,response:{authenticatorData:window.twofactorauthArrayBufferToBase64Url(cred.response.authenticatorData),clientDataJSON:window.twofactorauthArrayBufferToBase64Url(cred.response.clientDataJSON),signature:window.twofactorauthArrayBufferToBase64Url(cred.response.signature),userHandle:cred.response.userHandle?window.twofactorauthArrayBufferToBase64Url(cred.response.userHandle):''}};const verifyData=await window.twofactorauthJaxonPasskeyCall('verifyAuthentication',['" . $csrfEscaped . "',body]);if(verifyData&&verifyData.ok){window.location='runmodule.php?module=twofactorauth&op=resume';return;}alert('Passkey verification failed.');}catch(e){alert('Passkey operation failed.');}});})();</script>");
    }
}
...
{
    rawoutput("<script>(function(){window.twofactorauthPendingRequests=window.twofactorauthPendingRequests||{};window.twofactorauthHandleJaxonResponse=window.twofactorauthHandleJaxonResponse||function(requestId,payload){if(!requestId){return;}const entry=window.twofactorauthPendingRequests[requestId];if(!entry){return;}delete window.twofactorauthPendingRequests[requestId];entry.resolve(payload||{ok:false,error:'empty_response'});};window.twofactorauthJaxonPasskeyCall=window.twofactorauthJaxonPasskeyCall||function(method,args){return new Promise(function(resolve,reject){const namespace=window.Lotgd&&window.Lotgd.Async&&window.Lotgd.Async.Handler&&window.Lotgd.Async.Handler.TwoFactorAuthPasskey?window.Lotgd.Async.Handler.TwoFactorAuthPasskey:null;if(!namespace||typeof namespace[method]!=='function'){reject(new Error('Passkey async handler unavailable.'));return;}const requestId='tfa_'+Date.now()+'_'+Math.random().toString(16).slice(2);window.twofactorauthPendingRequests[requestId]={resolve:resolve,reject:reject};try{namespace[method].apply(namespace,[requestId].concat(args||[]));}catch(error){delete window.twofactorauthPendingRequests[requestId];reject(error);}});};})();</script>");
}
