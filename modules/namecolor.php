...
function namecolor_dohook(string $hookname, array $args): array
{
    ...
    switch ($hookname) {
        case "pointsdesc":
            ...
            $str = sprintf(
                $str,
                get_module_setting("initialpoints"),
...
function namecolor_run(): void
{
    ...
    if ($op == "namechange") {
        ...
        if ($rebuy) {
            ...
            output(
                "\"`&Because this will be your first name color change, it will cost you %s points.  Future name changes will only cost %s points.`7\"",
                $cost,
                get_module_setting('extrapoints')
            );
        }
