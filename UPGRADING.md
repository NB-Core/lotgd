...
- **Two-factor auth key derivation**:
  ...
  - **Fixed: that compatibility window locked roughly one account in 262 out of 2FA entirely.** The stored format is aes-256-cbc with no authentication tag, so decrypting with the wrong key fails only when the final block's PKCS#7 padding happens to be invalid — about 255 times in 256. The compatibility read tried the *current* key first and accepted any non-empty result, so a legacy-encrypted secret whose ciphertext fell in the remaining 0.4% "decrypted" into random bytes and the legacy key was never tried. Because the stored blob and both keys are fixed per account, this was permanent for whoever it landed on: every correct token refused, `failed_attempts` climbing toward the lockout, and retrying no help. The read now checks that the decrypted value can be a TOTP secret at all before accepting it. **An account that could not pass 2FA for no apparent reason should simply work after this upgrade** — no migration, no re-enrolment, nothing to run. Authenticating the stored ciphertext outright is the better long-term answer, but it changes the format and is a separate change.
