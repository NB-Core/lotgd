...
if ($op == "add2") {
    ...
    $sql = "SELECT name FROM " . \Lotgd\MySQL\Database::prefix("accounts") . " WHERE acctid=$id;";
    ...
    $sql = "UPDATE " . \Lotgd\MySQL\Database::prefix("accounts") . " SET donation=donation+'$points' WHERE acctid='$id'";
    \Lotgd\MySQL\Database::query($sql);
    modulehook("donation", array("id" => $id, "amt" => $points, "manual" => ($txnid > "" ? false : true)));

    if ($txnid > "") {
        $sql = "UPDATE " . \Lotgd\MySQL\Database::prefix("paylog") . " SET acctid='$id', processed=1 WHERE txnid='$txnid'";
        \Lotgd\MySQL\Database::query($sql);
...
if ($op == "") {
    $sql = "SELECT name,donation,donationspent FROM " . \Lotgd\MySQL\Database::prefix("accounts") . " WHERE donation>0 ORDER BY donation DESC LIMIT 25";
    $result = \Lotgd\MySQL\Database::query($sql);
    ...
    $sql = "SELECT name,acctid,donation,donationspent FROM " . \Lotgd\MySQL\Database::prefix("accounts") . " WHERE login LIKE '$search' or name LIKE '$search' LIMIT 100";
    $result = \Lotgd\MySQL\Database::query($sql);
