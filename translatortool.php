...
if ($op == "") {
    ...
    $sql .= "
		FROM " . \Lotgd\MySQL\Database::prefix("translations") . "
    ...
    if ($trans > "") {
        ...
        if (\Lotgd\MySQL\Database::numRows($result) == 0) {
            $sql = "INSERT INTO " . \Lotgd\MySQL\Database::prefix("translations") . " (language,uri,intext,outtext,author,version) VALUES ('" . LANGUAGE . "','$uri','$text','$trans','{$session['user']['login']}','$logd_version ')";
            $sql1 = "DELETE FROM " . \Lotgd\MySQL\Database::prefix("untranslated") .
                " WHERE intext='$text' AND language='" . LANGUAGE .
                "' AND namespace='$url'";
            \Lotgd\MySQL\Database::query($sql1);
            ...
            if ($row['intext'] == $text) {
                $sql = "UPDATE " . \Lotgd\MySQL\Database::prefix("translations") . " SET author='{$session['user']['login']}', version='$logd_version', uri='$uri', outtext='$trans' WHERE tid={$row['tid']}";
            } else {
                $sql = "INSERT INTO " . \Lotgd\MySQL\Database::prefix("translations") . " (language,uri,intext,outtext,author,version) VALUES ('" . LANGUAGE . "','$uri','$text','$trans','{$session['user']['login']}','$logd_version ')";
                $sql1 = "DELETE FROM " . \Lotgd\MySQL\Database::prefix("untranslated") . " WHERE intext='$text' AND language='" . LANGUAGE . "' AND namespace='$url'";
                \Lotgd\MySQL\Database::query($sql1);
            }
        } elseif (\Lotgd\MySQL\Database::numRows($result) > 1) {
        /* To say the least, this case is bad. Simply because if there are duplicates, you make them even more equal. But most likely you won't get this far, as the code itself should not produce duplicates unless you insert manually or via module the same row more than once*/
            ...
            $sql = "UPDATE " . \Lotgd\MySQL\Database::prefix("translations") . " SET author='{$session['user']['login']}', version='$logd_version', uri='$page', outtext='$trans' WHERE tid IN (" . join(",", $rows) . ")";
        }
    }
    ...
    $sql = "SELECT uri,count(*) AS c FROM " . \Lotgd\MySQL\Database::prefix("translations") . " WHERE language='" . LANGUAGE . "' GROUP BY uri ORDER BY uri ASC";
    $result = \Lotgd\MySQL\Database::query($sql);
    ...
    $sql = "SELECT * FROM " . \Lotgd\MySQL\Database::prefix("translations") . " WHERE language='" . LANGUAGE . "' AND uri='" . httpget("u") . "'";
    $result = \Lotgd\MySQL\Database::query($sql);
    if (\Lotgd\MySQL\Database::numRows($result) > 0) {
        $i = 0;
        while ($row = \Lotgd\MySQL\Database::fetchAssoc($result)) {
