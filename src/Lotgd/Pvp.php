...
/**
 * Collection of helper methods for player versus player combat.
 */
class Pvp
{
    /**
     * Display or apply the PvP immunity warning.
     */
    public static function warning(bool $dokill = false): void
    {
        global $session;
        ...
        if (
            ...
            if ($dokill) {
                ...
                output(
                    '`\$Warning!`^ Players are immune from Player vs Player (PvP) combat for their first %s days in the game or until they have earned %s experience, or until they attack another player.  If you choose to attack another player, you will lose this immunity!`n`n',
                    $days,
                    $exp
                );
            }
    ...
    /**
     * Build the list of potential PvP targets.
     */
    public static function playerList($location = false, $link = false, $extra = false, $sql = false): void
    {
        ...
        if ($sql === false) {
            ...
            $sql = 'SELECT acctid, name, race, alive, location, sex, level, laston, ' .
                'loggedin, login, pvpflag, clanshort, clanrank, dragonkills, ' . db_prefix('accounts') . '.clanid FROM ' .
                db_prefix('accounts') . ' LEFT JOIN ' . db_prefix('clans') . ' ON ' . db_prefix('clans') . '.clanid=' .
        ...
        foreach ($pvp as $row) {
            ...
            if ($row['clanshort'] > '' && $row['clanrank'] > CLAN_APPLICANT) {
                output_notl('%s&lt;`2%s%s&gt;`0 ', $clanrankcolors[ceil($row['clanrank'] / 10)], $row['clanshort'], $clanrankcolors[ceil($row['clanrank'] / 10)], true);
            }
    ...
    /**
     * Prepare a target array for PvP combat.
     *
     * @return array|false Target data or false when invalid.
     */
    public static function setupTarget($name)
    {
        ...
        $sql = 'SELECT name AS creaturename, level AS creaturelevel, weapon AS creatureweapon, dragonkills AS dragonkills,' .
            ...
            ' defense AS creaturedefense, loggedin, location, laston, alive, acctid, pvpflag, boughtroomtoday, race FROM ' . db_prefix('accounts') . " WHERE $where";
        $result = db_query($sql);
        if (db_num_rows($result) > 0) {
    ...
    /**
     * Process a victory in PvP combat.
     */
    public static function victory($badguy, string $killedloc, $options = false)
    {
        global $session;
    ...
    /**
     * Handle defeat in PvP combat.
     */
    public static function defeat($badguy, string $killedloc, $taunt, $options = false)
    {
        global $session;
