...
class TwoFactorAuthPasskey
{
    ...
    {
        // This used to fork on whether the module's token helper happened to be
        // loaded, falling back to a raw session read, because async/process.php
        // does not load module functions. A core class is reachable from both
        // paths, so there is one comparison again — and, unlike the module
        // helper, it does not create a token while checking one, which matters
        // here: the session lock may already be released.
        return Csrf::matches(Csrf::SCOPE_TWOFACTORAUTH, $csrfToken);
    }
