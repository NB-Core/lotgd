...
class TwoFactorAuthPasskey
{
    ...
    private function isCsrfValid(string $csrfToken): bool
    {
        ...
        if (\function_exists('twofactorauth_csrf_token')) {
            $expectedToken = (string) \twofactorauth_csrf_token();

            return $expectedToken !== '' && hash_equals($expectedToken, $csrfToken);
        }
        ...
        $expectedToken = (string) ($GLOBALS['session']['twofactorauth_csrf'] ?? '');
