...
$setup = array(
    ...
    "Note: this collects page and hook runtimes only. To show error detail to visitors use `^Show detailed error output`0 under Error Notification.,note",
    "expiredebug" => "Days to keep collected DEBUG mode runtimes? (0=infinite),int",
...
//      "If you have updates so severe that nobody should cause a query to vital tables then activate this. Best approach is to activate first the normal maintenance and after a few minutes activate the full maintenance.,note",
    ...
    "homeonline_minutes" => "Minutes for 'last X minutes' mode,range,1,480,1",
    ...
    "ONLY experienced admins should use cron jobbing here. Enabling this setting disables user-triggered newday processing; schedule cron.php to run regularly,note",
    ...
    "show_error_details" => "Show detailed error output (message, file, backtrace) to `\$ALL`0 visitors?,bool",
    "`\$WARNING!`0 Leave this off on a live server. Error detail exposes file paths and stack traces, which help an attacker map your installation. Administrators with megauser rights always see the detail regardless of this setting, so turn it on only to debug a fault that nobody can log in to reproduce.,note",
