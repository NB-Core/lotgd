...
/**
 * Miscellaneous helper methods dealing with server wide tasks.
 */
class ServerFunctions
{
    /**
     * Check if the server has reached its maximum number of online players.
     */
    public static function isTheServerFull(): bool
    {
        if (abs(getsetting('OnlineCountLast', 0) - strtotime('now')) > 60) {
            $sql = 'SELECT count(acctid) as counter FROM ' . db_prefix('accounts') .
                " WHERE locked=0 AND loggedin=1 AND laston>'" . date('Y-m-d H:i:s', strtotime('-' . getsetting('LOGINTIMEOUT', 900) . ' seconds')) . "'";
            $result = db_query($sql);
    ...
    /**
     * Reset all dragon kill points for one or more accounts.
     *
     * @param int|array|false $acctid Account id(s) or false for all
     */
