...
if (db_num_rows($result) > 0) {
    ...
    $motdauthname = translate_inline("Nobody");
}
...
if (db_num_rows($result) > 0) {
    ...
    $descauthname = translate_inline("Nobody");
}
...
if ($claninfo['clandesc'] != '') {
    ...
    modulehook("}collapse");
        ...
        $sql = "SELECT count(acctid) AS c, clanrank FROM " . db_prefix("accounts") . " WHERE clanid={$claninfo['clanid']} GROUP BY clanrank ORDER BY clanrank DESC";
        $result = db_query($sql);
        // begin collapse
