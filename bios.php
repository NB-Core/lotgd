...
if ($op == "block") {
       $sql = "UPDATE " . \Lotgd\MySQL\Database::prefix("accounts") . " SET bio='`iBlocked for inappropriate usage`i',biotime='9999-12-31 23:59:59' WHERE acctid=$userid";
...
if ($op == "unblock") {
       $sql = "UPDATE " . \Lotgd\MySQL\Database::prefix("accounts") . " SET bio='',biotime='" . DATETIME_DATEMIN . "' WHERE acctid=$userid";
...
$sql = "SELECT name,acctid,bio,biotime FROM " . \Lotgd\MySQL\Database::prefix("accounts") . " WHERE biotime<'9999-12-31' AND bio>'' ORDER BY biotime DESC LIMIT 100";
...
$sql = "SELECT name,acctid,bio,biotime FROM " . \Lotgd\MySQL\Database::prefix("accounts") . " WHERE biotime>'9000-01-01' AND bio>'' ORDER BY biotime DESC LIMIT 100";
$result = \Lotgd\MySQL\Database::query($sql);
